...
(function () {
  ...
  const GM_getValue = (key) => {
    try {
      return JSON.parse(localStorage.GM_VALUE)[key];
    } catch (error) {
      return null;
    }
  };
  const GM_setValue = (key, value) => {
    let v = {};
    try {
      v = JSON.parse(localStorage.GM_VALUE);
    } catch (error) {}
    v[key] = value;
    localStorage.GM_VALUE = JSON.stringify(v);
  };
  ...
  function registerMenuCommand() {
    ...
    menu_ID[menu_ID.length] = GM_registerMenuCommand(
      ...
      function () {
        ...
        window.GM_openInTab(
          "https://greasyfork.org/zh-CN/scripts/424246/feedback",
          { active: true, insert: true, setParent: true }
        );
      }
    );
  ...
  let DBSite = {
    ...
    replies: {
      ...
      pager: {
        ...
        nextLink: '//a[@class="page_current"]/following-sibling::a[1][@href]',
        pageElement:
          '//div[@id="Main"]//div[@class="box"]//div[@class="dock_area"] | //*[@id="Main"]//div[@class="box"]//div[@class="inner"] | //*[@id="Main"]//div[@class="box"]//div[@class="dock_area"][last()]/following-sibling::div[@class="cell"][1]',
        HT_insert: [
          '//div[@id="Main"]//div[@class="box"]//div[@class="cell"][last()]',
          1,
        ],
    ...
    reply: {
      ...
      pager: {
        ...
        HT_insert: [
          '//div[starts-with(@id, "r_")][last()]/following-sibling::div[@class="cell"][1]',
          1,
        ],
        replaceE: "css;#Main > .box > .cell[style] > table",
        scrollDelta: 1500,
    ...
    balance: {
      ...
      pager: {
        ...
        nextLink: '//a[@class="page_current"]/following-sibling::a[1][@href]',
        pageElement:
          "css;#Main .box > div:not(.cell) > table > tbody > tr:not(:first-child)",
  ...
  switch (location.pathname) {
    ...
    case "/balance": //       账户余额页
      ...
      break;
    default:
      if (location.pathname.indexOf("/go/") > -1) {
        // 分类主题页
        curSite = DBSite.go;
      } else if (location.pathname.indexOf("/t/") > -1) {
        // 帖子内容页
        if (menu_value("menu_pageLoading_reply"))
          curSite = DBSite.reply_positive; // 帖子内自动无缝翻页
  ...
  function qianDao() {
    let timeNow =
      ...
      "/" +
      new Date().getUTCDate(); // 当前 UTC-0 时间（V2EX 按这个时间的）
    if (location.pathname == "/") {
      //                               在首页
      let qiandao = document.querySelector(
        '.box .inner a[href="/mission/daily"]'
      );
      if (qiandao) {
        //                                            如果找到了签到提示
  ...
  function qianDaoStatus_(timeNow) {
    GM_xmlhttpRequest({
      ...
      onload: function (response) {
        let html = ShowPager.createDocumentByString(response.responseText);
        if (html.querySelector('input[value^="领取"]')) {
          //     还没有签到...
  ...
  // 回到顶部（右键左右两侧空白处）
  function backToTop() {
    document.getElementById("Wrapper").oncontextmenu = document.querySelector(
      "#Wrapper > .content"
  ...
  function linksToImgs() {
    ...
    Array.from(links).forEach(function (_this) {
      if (
        /^https.*\.(?:jpg|jpeg|jpe|bmp|png|gif)/i.test(_this.href) &&
        !/<img\s/i.test(_this.innerHTML)
      ) {
  ...
  // 快速回复（双击左右两侧空白处）
  function quickReply() {
    document.getElementById("Wrapper").ondblclick = document.querySelector(
      "#Wrapper > .content"
  ...
  function linksBlank() {
    ...
    Array.from(document.links).forEach(function (_this) {
      if (
        _this.onclick ||
        _this.href.slice(0, 4) != "http" ||
        _this.href.indexOf("#;") > -1 ||
        _this.href.indexOf("night/toggle") > -1 ||
        _this.href.indexOf("/favorite") > -1
      ) {
        _this.target = "_self";
      }
    });
        ...
        for (const target of mutation.addedNodes) {
          ...
          if (target.tagName === "A") {
            if (
              ...
              target.href.indexOf("night/toggle") > -1 ||
              target.href.indexOf("/favorite") > -1
            ) {
              target.target = "_self";
            }
          } else {
  ...
  function addChangesLink() {
    let links = document.querySelector("#Main .box .inner:last-child");
    if (!links) return;
      ...
      windowScroll(function (direction, e) {
        if (direction === "down") {
          ...
          if (
            ...
            document.documentElement.clientHeight + scrollTop + scrollDelta
          ) {
            ...
            try {
              ...
              if (pageElems.length >= 0) {
                ...
                if (curSite.function && curSite.function.before) {
                  if (curSite.function.parameter) {
                    // 如果指定了参数
                    pageElems = curSite.function.before(
                      curSite.function.parameter
                    );
                  } else {
